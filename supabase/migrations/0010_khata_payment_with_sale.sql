-- =============================================================
-- Costmatic — Migration 0010: pay old khata from the same POS cash
--
-- Shop reality: an udhaar customer buys again and hands over more cash than
-- this bill. The cashier must be able to put the extra straight onto the old
-- balance instead of returning it as change — in ONE transaction with the sale.
--
-- create_sale v5 gains p_khata_payment (extra cash applied to the customer's
-- outstanding balance). It is clamped server-side to what is actually owed
-- AFTER this sale's udhaar is posted, so the balance can never go negative and
-- a client can never invent a payment larger than the debt.
--
-- Backward compatible: p_khata_payment defaults to 0 → every existing caller
-- (including the offline queue's older payloads) behaves exactly as before.
-- =============================================================

drop function if exists public.create_sale(uuid, date, numeric, numeric, numeric, numeric, numeric, numeric, text, uuid, jsonb, uuid, text, boolean);
...
create or replace function public.create_sale(
  ...
  p_allow_override boolean default false, p_khata_payment numeric default 0)
...
begin
  ...
  if p_client_id is not null then
    ...
    -- Idempotent replay (offline queue): the khata payment was already applied
    -- on the first run, so never apply it twice.
    if found then return jsonb_build_object('sale_id', v_sale_id, 'invoice_no', v_invoice_no, 'khata_paid', 0); end if;
  end if;
  ...
  -- ── Extra cash applied to the old khata (server-clamped) ──
  -- Runs AFTER this sale's udhaar is posted, so the clamp is against the
  -- customer's true outstanding balance and can never overpay into credit.
  v_khata := round(greatest(coalesce(p_khata_payment, 0), 0), 2);
  if v_khata > 0 then
    if p_customer_id is null then
      raise exception 'Khata payment requires a customer';
    end if;
    select greatest(coalesce(current_balance, 0), 0) into v_balance
      from customers where id = p_customer_id for update;
    v_khata := least(v_khata, coalesce(v_balance, 0));
    if v_khata > 0 then
      insert into customer_ledger (customer_id, type, amount, ref_sale_id, date, note, created_by)
      values (p_customer_id, 'payment', -v_khata, v_sale_id, p_date,
              'Khata payment with ' || v_invoice_no, v_uid);
      update customers set current_balance = current_balance - v_khata where id = p_customer_id;
    end if;
  end if;

  return jsonb_build_object('sale_id', v_sale_id, 'invoice_no', v_invoice_no, 'khata_paid', v_khata);
end;
$$;

revoke execute on function public.create_sale(uuid, date, numeric, numeric, numeric, numeric, numeric, numeric, text, uuid, jsonb, uuid, text, boolean, numeric) from public, anon;
grant  execute on function public.create_sale(uuid, date, numeric, numeric, numeric, numeric, numeric, numeric, text, uuid, jsonb, uuid, text, boolean, numeric) to authenticated;
